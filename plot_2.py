...
def plot_3d(sc, history):
    ...
    for i in range(1000):
        ...
        HV_sc1[i,:] = config_SC.HV + (sc[0,:]-config_SC.HV)*i/1000
        HV_sc2[i,:] = config_SC.HV + (sc[1,:]-config_SC.HV)*i/1000
        HV_sc3[i,:] = config_SC.HV + (sc[2,:]-config_SC.HV)*i/1000
        HV_sc4[i,:] = config_SC.HV + (sc[3,:]-config_SC.HV)*i/1000
    ...
    lim = np.max(np.abs(np.vstack([config_SC.SV, config_SC.HV, sc]))) * 1.2
    ...
    ax.scatter(*config_SC.SV, s=100, c='r', label='SV')
    ax.scatter(*config_SC.HV, s=100, c='k', label='true HV')
