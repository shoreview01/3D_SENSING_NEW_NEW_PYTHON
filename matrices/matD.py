...
def matD_caliter(C, alpha, theta, psi, phi, Qprev, wprev, rho, vprev, d1, P, c):
    ...
    for idx in range(1, P):
        delta_x = Qprev * C[idx-1,0] \
                  ...
                  - vprev[0] * triop(1, theta[0], alpha[0]) \
                  + vprev[idx] * triop(1, theta[idx], alpha[idx])
        D.append(delta_x)
    for idx in range(1, P):
        delta_y = Qprev * C[N+idx-1,0] \
                  ...
                  - vprev[0] * triop(2, theta[0], alpha[0]) \
                  + vprev[idx] * triop(2, theta[idx], alpha[idx])
        D.append(delta_y)
    for idx in range(1, P):
        delta_z = Qprev * C[2*N+idx-1,0] \
                  - vprev[0] * np.cos(theta[0]) \
                  + vprev[idx] * np.cos(theta[idx]) \
